...
private:
	...
	void siftUp(int pos) {
		while (pos != 0 && elements[pos] < elements[parent(pos)]) { // No raíz y que yo siga siendo menor que mi padre
			swap(pos, parent(pos));
			pos = parent(pos);
		}
	}
	void siftDown(int pos) {
		while (!isLeaf(pos) && elements[pos] > elements[minChild(pos)]) { // Si no soy hoja y soy mayor que el menor de mis hijos
